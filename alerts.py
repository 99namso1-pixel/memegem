...
def build_alert(gem, rank=1):
    ...
    lines = [
        ...
        *(
            ...
            else [f"😴 DEAD VOL ACCUM — {getattr(gem,'stealth_hours',0)}h vol flat, whale đang hold!",
                  f"💡 Vào trước catalyst — top10 thấp + liq tốt", f""]
            if getattr(gem,'stealth_accum',False) and getattr(gem,'vol_accel',99)<0.5
